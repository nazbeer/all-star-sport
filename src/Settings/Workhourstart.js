...
const Workhourstart = () => {
    ...
    const getWorkhourstart = async () => {
     
      try {
            const response = await axios.get('https://allstarstports.arenacapital.org/api/v1/settings/get')
            .then((res)=>{
            //    console.log('res')
                console.log(res?.data.data?.work_hour_start)
            if (Array.isArray(res?.data?.data?.work_hour_start)) {
                // console.log(res?.data?.data);
                setData(res?.data?.data?.work_hour_start);
            } else if (typeof res?.data?.data?.work_hour_start === 'object') {
                if (Array.isArray(res?.data?.data?.work_hour_start)) {
                setData(res?.data?.data?.work_hour_start);
                } 
            } 
            ...
            <div className='col-md-6'>
                <div className='card'>
                    <div className='card-header'>
                       <h4>Work hour start Settings</h4>
                    </div>
